...
#Directory settings
db <- org.Hs.eg.db
filenames <- list.files(path = "./data/GSE186341/", pattern = '*.csv', , full.names = T)
megacounts <- tibble()
megacontrast <- tibble()
megametadata <- tibble()
...
write_tsv(megacounts, file = 'GSE186341__countdata.tsv')
write_tsv(megacontrast, file = 'GSE186341__contrast.tsv')
write_tsv(megametadata, file = 'GSE186341__metadata.tsv')
