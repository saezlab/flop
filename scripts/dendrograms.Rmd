...
```{r}
boxplot_k <- ggplot(rand_results_long, aes(x=factor(k), y=scores, fill=factor(k))) +
  ggdist::stat_halfeye(
    adjust = .5, 
    width = .6, 
    .width = 0, 
...
aligned_plots <- egg::ggarrange(boxplot_pipelines, boxplot_k, ncol = 2)
final <- ggdraw() + draw_plot(aligned_plots, 0, 0, 1, 1) +
  draw_plot_label('A', 0, 1)+
  draw_plot_label('B', 0.52, 1)
save_plot(filename = './results/plots/rand_index_plots.pdf', plot = final, device='pdf', dpi=300, base_height=10, base_width =15)
```
